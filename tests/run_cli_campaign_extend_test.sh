...
mkdir -p "$WORKDIR/baseline" "$WORKDIR/campaign" "$WORKDIR/multi"
for dir in baseline campaign multi; do
...
(
  ...
  awk 'NR==6{$0="200"}1' input.param > input.param.tmp
  mv input.param.tmp input.param
  if TUTRAST_KMC_SEED=42 "$BIN" --campaign-dir ./campaign_state > multi_extend.log 2>&1; then
    echo "Expected multi-run step extension to be rejected"
    exit 1
  fi
  grep -q "supported only for single-run campaigns" multi_extend.log
)
